				...
				<div id="content">
					...
					if ( $query->have_posts() ) {
						while ( $query->have_posts() ) {
							...
							$id        = get_the_ID();
							$status    = get_post_meta( $id, 'event_available', true ) ? null : '<span class="sold-out">Ausgebucht</span>';
							...
							printf(
								'<div class="course">
                                    <a href="%1$s" class="image" style="display:block; min-height: 300px; background:url(%2$s); background-size:cover; background-position:center;"></a>
                                    %3$s
